...
public class DeadlockExercise extends Exercise {
    ...
    private final HODApi hodApi = new HODApi();
    ...
    class CreateCompanyAndJob implements Runnable {
        public void run() {
            while (true) {
                try {
                    hodApi.createCompanyAndJob("wang", "head of awesome", 3, 4, "sdfettre");
            ...
            while (true) {
                try {
                    ...
                    if (companyDeletedCount.intValue() < companyCreatedCount.intValue()) {
                        hodApi.deleteCompanyAndJobById(companyDeletedCount.incrementAndGet() + 1);
                    } else {
                        Thread.sleep(100);
                    }
                } catch (ClientProtocolException e) {
